...
public class TestMainViewModel
{
  ...
  public void Setup()
  {
    ...
    _mockTourRepository.Setup(x => x.GetAddressById(It.IsAny<int>())).Returns(new AddressEntity
      { Id = 0, City = "Test", Zip = 1234, Housenumber = "1", Street = "Test" });
  ...
  public void TourList_NowExpandedTour_ShouldHandleEvent()
  {
    ...
    var tourViewModel = new TourViewModel(_mockTourRepository.Object, tourModel, _mockMapCreator.Object,
      _mockMessageBoxService.Object, _mockOpenRouteService.Object);
    var _mockChangeTourCommand = new Mock<ICommand>();
    var methodInfo = typeof(MainViewModel).GetMethod("TourList_NowExpandedTour",
      BindingFlags.NonPublic |
      BindingFlags.Instance); // Refelxion to run private "TourList_NowExpandedTour", invoked by event
  ...
  {
    // Arrange          
    var transportType = TransportType.car;
    var start = new[] { 16.378317, 48.238992 };
    var end = new[] { 12.99528, 47.82287 };
    var tourId = 1;

    _mockTourRepository.Setup(r => r.ImportTourAsync(_filePath))
      .ReturnsAsync((transportType, start, end, tourId));

    _mockMapCreator.Setup(x => x.DownloadMapFromApi(It.IsAny<TransportType>(),
